...
static int rl_gui_object_radio_button_initialize(
		struct rl_gui_object_radio_button_instance *instance
);


static void rl_gui_object_radio_button_deinitialize(
		struct rl_gui_object_radio_button_instance *instance
);
...
static GLuint rl_gui_shader_vertex_create(
...
static int rl_gui_common_uniform_locations_find(
		struct rl_gui_common_uniform_locations *locations, 
		const GLuint program
);

static void rl_gui_common_uniform_locations_set(
...
static int rl_gui_surface_initialize(
		struct rl_gui_surface *surface
);

static void rl_gui_surface_deinitialize(
		struct rl_gui_surface *surface
);

static void rl_gui_surface_render(
		const struct rl_gui_surface *rl_gui_surface
);

/* Code generated by chatgpt */
static uint32_t utf8_decode(const char *s, uint32_t *codepoint)
{
...
static uint32_t index_to_utf8_offset(const char *str, uint32_t index)
{
	uint32_t offset = 0;
	for(uint32_t j = 0; j < index; j++)
	{
		uint32_t c = 0;
		uint32_t c_inc = utf8_decode(&str[offset], &c);
		if(c_inc == 0){
			break;
		}
		offset += c_inc;	
...
static uint32_t utf8_offset_to_count(const char *str, uint32_t offset)
{
	uint32_t str_index = 0;	
	uint32_t char_count = 0;
	while(str_index < offset)
	{
		
		uint32_t c = 0;
		uint32_t c_inc = utf8_decode(&str[str_index], &c);
		if(c_inc == 0){
			break;
		}
		str_index += c_inc;	
...
)
{
	...
	for(uint32_t i = 0; i < entry_count; i++)
	{
		...
		if(list->selected > 0 && list->selected_index == i)
		{
			...
			uint32_t utf8_byte_index_1 = index_to_utf8_offset(entry[i], overflow_count_index);


			uint32_t utf8_byte_index_2 = index_to_utf8_offset(entry[i], overflow_count);
...
static GLuint 
rl_gui_shader_vertex_create_from_memory(
...
static int 
rl_gui_surface_initialize(struct rl_gui_surface *surface)
{
...
static void 
rl_gui_surface_deinitialize(struct rl_gui_surface *surface)
{
...
void static 
rl_gui_surface_render(const struct rl_gui_surface *rl_gui_surface)
{
...
{
	uint32_t char_count = utf8_offset_to_count(text, text_length);
...
)
{
	...
	if(field->blank_object.clicked > 0)
	{
		...
		while(overflow_offset_iter < end_offset)
		{
			...
			{
				field->cursor_index = overflow_count +  i;
				index_found = 1;
				break;	
			}
	...
	{
		uint32_t overflow_offset_iter  = overflow_offset;
		while(overflow_offset_iter < cursor_offset)
		{
			uint32_t c = 0;
			uint32_t c_inc = utf8_decode(&input_buffer[overflow_offset_iter], &c);
			if(c_inc == 0){
...
)
{
	...
	while(str_index  < text_length)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_R8,
			       	...
			       	0, GL_RED, GL_UNSIGNED_BYTE, font->face->glyph->bitmap.buffer);
		...
		float width_mm = (float)font->face->glyph->bitmap.width * mm_per_pixel_x;
		float height_mm = (float)font->face->glyph->bitmap.rows * mm_per_pixel_y;

		
		struct rl_gui_result result_tmp = result;	
		result_tmp.x_mm = r.x_mm + font->face->glyph->bitmap_left * mm_per_pixel_x;

		result_tmp.y_mm = r.y_mm - (font->face->glyph->bitmap.rows- font->face->glyph->bitmap_top) * mm_per_pixel_y - delta_y_max_mm;
		...
		r.x_mm += (font->face->glyph->advance.x >> 6) * mm_per_pixel_x;
...
static int
rl_gui_object_radio_button_initialize(struct rl_gui_object_radio_button_instance *instance)
{
	uint8_t vertex_source[] = 
				"#version 430 core \n "
				"layout(location=0) in vec2 v_position; \n"
...
static void 
rl_gui_object_radio_button_deinitialize(struct rl_gui_object_radio_button_instance *instance)
{
	glDeleteProgram(instance->shader_program);
}
